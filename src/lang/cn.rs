...
pub static ref T: std::collections::HashMap<&'static str, &'static str> =
    [
        ...
        ("Direct grace", "直连窗口"),
        ("Relay deadline", "中继期限"),
        ("UDP wait min", "UDP 等待下限"),
        ("UDP wait max", "UDP 等待上限"),
        ("UDP budget", "UDP 预算"),
        ("EasySym window", "对称窗口"),
        ("HardSym fallback", "对称回退"),
        ("Path memory", "路径记忆"),
        ("Circuit breaker", "断路阈值"),
        ("Orchestrator v2", "编排 V2"),
        ("NAT profile", "NAT 画像"),
        ("Non-public default", "非公网默认"),
        ("Prefer UDP direct. Disable to always relay. Default on.", "优先尝试 UDP 直连。关闭后直接走中继。默认开启。"),
        ("Allow IPv6 direct attempt. IPv6 unavailable does not affect IPv4.", "允许 IPv6 直连尝试。IPv6 不可用时不影响 IPv4。"),
        ("Direct total budget. Too small forces relay. Default 8000ms. Range 1000-60000.", "直连总时长上限。过小易中继。默认 8000ms。范围 1000-60000。"),
        ("Direct grace window. Relay is not committed within this window. Default 600ms. Range 100-5000.", "直连独占窗口，窗口内不提交中继。默认 600ms。范围 100-5000。"),
        ("Relay commit deadline. Default 1800ms. Range 200-10000.", "最晚中继提交时刻。默认 1800ms。范围 200-10000。"),
        ("UDP port ready min. Actual wait is RTT/2 clamped. Default 250ms. Range 50-5000.", "UDP 端口就绪等待下限。实际等待=RTT/2并夹取。默认 250ms。范围 50-5000。"),
        ("UDP port ready max. Actual wait is RTT/2 clamped. Default 1200ms. Range 50-5000.", "UDP 端口就绪等待上限。实际等待=RTT/2并夹取。默认 1200ms。范围 50-5000。"),
        ("UDP probe packet budget. Default 32. Range 4-256.", "UDP 探测包预算。默认 32。范围 4-256。"),
        ("EasySym prediction window. Default 7. Range 1-32.", "EasySym 预测窗口大小。默认 7。范围 1-32。"),
        ("HardSym fast fallback threshold. Default 300ms. Range 100-5000.", "HardSym 快速回退阈值。默认 300ms。范围 100-5000。"),
        ("Path cache TTL. Default 60s. Range 5-3600.", "路径缓存有效期。默认 60s。范围 5-3600。"),
        ("Direct failure threshold for circuit breaker. Default 3. Range 1-10.", "直连失败次数达到阈值后缩短预算。默认 3。范围 1-10。"),
        ("New connection orchestrator. Disable to fallback legacy.", "连接编排新逻辑开关。关闭会回退旧流程。"),
        ("Enable local NAT profile strategy. Disable to fallback legacy.", "启用本地 NAT 画像与置信度策略。关闭回退旧判断。"),
        ("Enable EasySym predicted ports.", "启用 EasySym 预测端口。"),
        ("Enable path memory and failure hints.", "启用路径记忆与失败熔断提示。"),
        ("Non-public relay default disables UDP/IPv6 when unset.", "非公网中继时，未设置则默认关闭 UDP/IPv6 打洞。"),
        ("Disable UDP completely. Use TCP/relay only.", "完全禁用 UDP，仅走 TCP/中继。"),
        ("Invalid value", "无效数值"),
        ("Empty = default", "空=默认"),
        ("Continue with {}", "使用 {} 登录"),
        ("Display Name", "显示名称"),
